...
def make_dataset(dataset, n_labeled, n_unlabeled, with_bias=False, resample_model=""):
    def make_PU_dataset_from_binary_dataset(x, y, labeled=n_labeled, unlabeled=n_unlabeled, bias=with_bias, resamplemodel=resample_model):
        labels = np.unique(y)
        ...
        Y = np.asarray(np.concatenate(
            (np.ones(n_lp), -np.ones(n_u))), dtype=np.int32)
        perm = np.random.permutation(len(Y))
        X, Y = X[perm], Y[perm]
        return X, Y, prior
    ...
    def make_PN_dataset_from_binary_dataset(x, y):
        ...
        Y = np.asarray(np.concatenate(
            (np.ones(n_p), -np.ones(n_n))), dtype=np.int32)
        perm = np.random.permutation(len(Y))
        X, Y = X[perm], Y[perm]
        return X, Y
...
def load_dataset(dataset_name, n_labeled, n_unlabeled, batchsize, with_bias=False, resample_model=""):
    print("==================")
    print("loading data...")
    ...
    XYtrain, XYtest, prior, dim = make_dataset(
        ((trainX, trainY), (testX, testY)), n_labeled, n_unlabeled, with_bias, resample_model)

    XYtrainLoader = to_dataloader(XYtrain[0], XYtrain[1], batchsize)
    XYvalidLoader = to_dataloader(XYtrain[0], XYtrain[1], batchsize)

    XYtestLoader = to_dataloader(XYtest[0], XYtest[1], batchsize)
